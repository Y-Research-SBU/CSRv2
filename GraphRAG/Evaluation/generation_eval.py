...
class SentenceTransformerEmbeddings(Embeddings):
    """Custom Embeddings wrapper for SentenceTransformer that implements Langchain's Embeddings interface"""
    
    def __init__(self, model: SentenceTransformer, output_dim: Optional[int] = None):
        self.model = model
...
async def main(args: argparse.Namespace):
    ...
    if args.mode == "API":
        ...
        print(f"Loading SentenceTransformer model from {args.embedding_model}...")
        sentence_transformer = SentenceTransformer(args.embedding_model)
        embedding = SentenceTransformerEmbeddings(
            model=sentence_transformer,
            output_dim=args.embed_dim
        )
        print(f"✅ Loaded SentenceTransformer model with output_dim={args.embed_dim}")
        ...
        # Initialize the SentenceTransformer embedding model for ollama mode
        print(f"Loading SentenceTransformer model from {args.embedding_model}...")
        sentence_transformer = SentenceTransformer(args.embedding_model)
        embedding = SentenceTransformerEmbeddings(
            model=sentence_transformer,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Evaluate RAG performance using various metrics with SentenceTransformer",
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to SentenceTransformer model directory"
    )
