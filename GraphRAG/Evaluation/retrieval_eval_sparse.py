...
async def evaluate_dataset(
    ...
    avg_results = {
        "context_relevancy": np.nanmean(results["context_relevancy"]) if results["context_relevancy"] else 0.0,
        "evidence_recall": np.nanmean(results["evidence_recall"]) if results["evidence_recall"] else 0.0
    }
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Evaluate RAG retrieval performance with SparseEncoder",
    ...
    parser.add_argument(
        "--output_file", 
        type=str,
        default="retrieval_results_sparse.json",
